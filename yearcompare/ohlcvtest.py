...
logging.basicConfig(filename='220326test.log', level=logging.INFO, format='%(message)s')
...
def hihi(k=0.8, target_v = 0.2, am = 15, m = 5):
    ...
    df.to_excel("/Users/sugang/Documents/GitHub/backtest/220326/data_1h_b.xlsx")
    return s, cagr, mdd
...
for h in hours:
    df = pd.read_excel(f'/Users/sugang/Documents/GitHub/backtest/220326/data_{h}.xlsx', index_col=0)
    for k in ks:
        for v in vs:
            for am in amount:
                for m in ma:
                    ...
                    logging.info(f'{h}, {k}, {v}, {am}, {m}, {cagr}, {mdd}')
